			...
			<table class="table table-sm table-bordered">
				...
				<tbody>
					<?php
						...
						$start = 5 * ($page -1);
						$getEvent = $db->prepare('SELECT * , events.id as event_id FROM events , groups WHERE events.group_id=groups.id ORDER BY events.created desc LIMIT ?,5 ;');
						$getEvent->bindParam(1 , $start , PDO::PARAM_INT);
						$getEvent->execute();
						while($eventList = $getEvent->fetch()):
					?>
						<tr>
							<td class="eventList_title"><?php print h($eventList['title']); ?></td>
							<td class="eventList_start"><?php print date("Y/m/d H:i" , strtotime(h($eventList['start']))); ?></td>
							<td class="eventList_place"><?php print h($eventList['place']); ?></td>
							<td class="eventList_group">
								<?php
									$arrayGroupList = explode(',',$eventList['group_id']);
							...
							<td class="eventList_detail"><a class="btn btn-outline-primary btn-sm" role="button" href="detail_event.php?view=<?php print h($eventList['event_id']); ?>&page=<?php print $page; ?>">開く</a></td>
						</tr>
					<?php endwhile; ?>
					...
					<nav>
						<?php
							$counts = $db->query('SELECT COUNT(*) AS cnt FROM events');
							$count = $counts->fetch();
						...
						<ul class="pagination mb-2">
							<?php if($page >= 2): ?>
								<li class="page-item"><a class="page-link" href="list_event.php?page=<?php print($page-1); ?>"><</a></li>
							<?php else: ?>
							...
							<?php for($i=1; $i <= $max_page; $i++): ?>
								...
								<?php else: ?>
									<li class="page-item"><a class="page-link" href="list_event.php?page=<?php print $i; ?>"><?php print $i; ?></a></li>
								<?php endif; ?>
							<?php endfor; ?>
